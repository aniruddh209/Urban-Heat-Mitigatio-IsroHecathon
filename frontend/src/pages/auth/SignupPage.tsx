...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem('user', JSON.stringify({ name: form.name, role: form.role, email: form.email }));
      navigate('/dashboard');
    }, 1000);
  };

  const inputClass = "w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3 text-white text-[13px] focus:outline-none focus:border-[#3B82F6]/30 focus:bg-white/[0.05] transition-all placeholder:text-[#334155]";

  return (
    <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden" style={{ background: '#030712' }}>
      <div className="absolute top-1/4 right-1/4 w-[500px] h-[500px] rounded-full blur-[120px] opacity-[0.05]" style={{ background: '#8B5CF6' }} />
      <div className="absolute bottom-1/4 left-1/4 w-[400px] h-[400px] rounded-full blur-[120px] opacity-[0.06]" style={{ background: '#3B82F6' }} />
      ...
      <div className="w-full max-w-[420px] relative z-10 animate-fade-in-up">
        ...
        <div className="rounded-2xl p-8" style={{ background: 'rgba(17,24,39,0.75)', border: '1px solid rgba(255,255,255,0.06)', backdropFilter: 'blur(40px)' }}>
          <div className="text-center mb-7">
            <h1 className="text-[22px] font-bold text-white mb-2">Create account</h1>
            <p className="text-[13px] text-[#64748B]">Join the climate intelligence platform</p>
          </div>

          <form onSubmit={handleSignup} className="space-y-4">
            <div>
              <label className="block text-[12px] font-medium text-[#94A3B8] mb-2">Full Name</label>
            ...
            <div>
              ...
              <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                className={inputClass} placeholder="you@email.com" required />
            </div>
            <div>
            ...
            <div>
              <label className="block text-[12px] font-medium text-[#94A3B8] mb-2">Role</label>
              <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}
                className={inputClass}>
                <option value="public">Public User</option>
                <option value="researcher">Researcher</option>
                <option value="government_planner">Government Planner</option>
              </select>
            </div>
          ...
          <p className="text-center text-[13px] text-[#475569] mt-6">
            Already have an account? <Link to="/login" className="text-[#3B82F6] hover:text-[#60A5FA] transition-colors font-medium">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
